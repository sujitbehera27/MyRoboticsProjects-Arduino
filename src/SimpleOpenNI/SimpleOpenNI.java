...
public class SimpleOpenNI extends ContextWrapper implements SimpleOpenNIConstants {
	...
	public static String getLibraryPathWin() {
		 ...
		 * SimpleOpenNI.class.getResource("SimpleOpenNI.class");
		 * //log.info("url = " + url); if (url != null) { // Convert URL to
		 * string, taking care of spaces represented by the "%20" // string.
		 * String path = url.toString().replace("%20", " "); int n0 =
		 * path.indexOf('/');
		 * 
	...
	*/
	...
	protected void updateDepthRealWorld() {
		...
		/*
		 * float[] depth3dArray = depthMapRealWorldA(); int index=0; for(int
		 * i=0;i < _depthMapRealWorld.length;i++) {
		 * _depthMapRealWorld[i].set(depth3dArray[index++],
		 * depth3dArray[index++], depth3dArray[index++]); }
		 */
	 ...
	 * float fTime) { try { _createHandsMethod.invoke(_handCbObject, new
	 * Object[] { (int)nId,new
	 * PVector(pPosition.getX(),pPosition.getY(),pPosition.getZ()),fTime}); }
	 * catch (Exception e) {} }
	 * 
	 * protected void onUpdateHandsCb(long nId, XnPoint3D pPosition, float
	 * fTime) { try { _updateHandsMethod.invoke(_handCbObject, new Object[] {
	 * (int)nId,new
	 * PVector(pPosition.getX(),pPosition.getY(),pPosition.getZ()),fTime}); }
	 * catch (Exception e) {} }
	 * 
	 * protected void onDestroyHandsCb(long nId, float fTime) { try {
	 * _destroyHandsMethod.invoke(_handCbObject, new Object[] {
	 * (int)nId,fTime}); } catch (Exception e) {} }
	 * 
	 * protected void onRecognizeGestureCb(String strGesture, XnPoint3D
	 * pIdPosition, XnPoint3D pEndPosition) { try {
	 * _recognizeGestureMethod.invoke(_gestureCbObject, new Object[] {
	 * strGesture, new
	 * PVector(pIdPosition.getX(),pIdPosition.getY(),pIdPosition.getZ()), new
	 * PVector(pEndPosition.getX(),pEndPosition.getY(),pEndPosition.getZ()) });
	 * } catch (Exception e) {} }
	 * 
	 * protected void onProgressGestureCb(String strGesture, XnPoint3D
	 * pPosition, float fProgress) { try {
	 * _progressGestureMethod.invoke(_gestureCbObject, new Object[] {
	 * strGesture, new
	 ...
	 * PVector(ptPosition.getX(),ptPosition.getY(),ptPosition.getZ()) }); }
	 * catch (Exception e) {} }
	 * 
